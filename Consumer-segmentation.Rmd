...
p3 <- fviz_cluster(km2, geom = "point",  data = BathSoap2[,c(1:10,13:21,31:45,47)]) + ggtitle("k = 2 for combined data")
...
Based on this we see that cluster 3 which is a combination of bothe clusters gives the higest degree of separation between clusters making it the best segmentation criteria.
...
```{r}
#Model Prediction using the best cluster on a sample test data of the firrst 40 rows.
library(FNN)
soapdata1<-soap.data1[1:40, -c(22:30,46)]
pred.knn <- get.knnx(fit.km2$centers, soapdata1, 1)$nn.index[,1]
